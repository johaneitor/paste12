...
##############################################################################
# 2) Limpiar indentación rota y normalizar create_note + GET/POST /api/notes
##############################################################################
say "Normalizando backend/routes.py (imports, indent y endpoints /api/notes)"
...
# Asegurar import de fingerprint al inicio
...
# Quitar líneas sueltas (top-level) que rompen indentación
s = re.sub(r'(?m)^\s*author_fp\s*=\s*client_fingerprint\(\)\s*,\s*$', '', s)

# Detectar el "bp" (nombre del blueprint) desde algún decorador tipo @xxx.route
m_bp = re.search(r'@([A-Za-z_]\w*)\.route\(', s)
bp = m_bp.group(1) if m_bp else None

# Eliminar cualquier definición previa de create_note y list_notes para reescribir limpio
s = re.sub(r'(?ms)@[\w\.]+\s*\.route\([^\)]*["\']/api/notes["\'][^\)]*\)\s*def\s+create_note\s*\(.*?\n\s*return[^\n]*\n\s*\)\s*', '', s)
s = re.sub(r'(?ms)@[\w\.]+\s*\.route\([^\)]*["\']/api/notes["\'][^\)]*\)\s*def\s+list_notes\s*\(.*?\n\s*return[^\n]*\n\s*\)\s*', '', s)

# Construir decoradores según se detecte blueprint o no
if bp:
    dec_get  = f"@{bp}.route('/api/notes', methods=['GET'])"
    dec_post = f"@{bp}.route('/api/notes', methods=['POST'])"
else:
    # fallback (aplicaciones sin blueprint): intentamos con 'app'
    dec_get  = "@app.route('/api/notes', methods=['GET'])"
    dec_post = "@app.route('/api/notes', methods=['POST'])"
...
def list_notes():
    ...
    page = max(1, page)
    try:
        # Requiere Note y _note_json en el contexto ya importado por tu router
...
# Insertamos el bloque al final del archivo
if not s.endswith("\n"): s += "\n"
s += block + "\n"

# Limpieza final: compactar múltiples líneas en blanco
s = re.sub(r'\n{3,}', '\n\n', s)
...
# Comprobar sintaxis
if ! python -m py_compile "$ROUTES" 2>>"$REPORT"; then
  warn "Python no pudo compilar routes.py — revisa $REPORT"; sed -n '1,120p' "$REPORT"; exit 1;
fi

##############################################################################
# 3) Hook before_insert (rellena author_fp si faltara)
##############################################################################
say "Instalando hook before_insert (relleno author_fp)"
cat > "$HOOK" <<'PY'
...
# Asegurar import del hook y limiter default + bind PORT (si aplica)
...
##############################################################################
# 4) Modelo y DB: asegurar author_fp mapeado y en la tabla
##############################################################################
say "Verificando modelo Note y mapeo de author_fp"
if [ -f "$MODELS" ] && ! grep -q "author_fp" "$MODELS"; then
  # Insertamos la columna en class Note
  python - "$MODELS" <<'PY'
import sys,re
p=sys.argv[1]
s=open(p,'r',encoding='utf-8').read()
...
if m:
    ...
    s = s[:at] + line + s[at:]
    open(p,'w',encoding='utf-8').write(s)
    print("[OK] models.py patched with author_fp")
else:
    print("[!] No encontré class Note en models.py")
PY
fi

if command -v sqlite3 >/dev/null 2>&1; then
  if [ -f "$DB" ]; then
    say "Chequeando/ajustando esquema SQLite"
    ...
    warn "No encontré $DB; omito migración de tabla note"
  fi
else
  warn "sqlite3 no instalado; no puedo migrar DB"
fi

##############################################################################
# 5) Reinicio local + smoke tests + reporte
##############################################################################
say "Reiniciando run.py (nohup) y escribiendo logs en ./.tmp/"
...
say "Tails → .tmp/paste12.log y .tmp/author_fp_hook.log"
tail -n 40 "$LOG"     >>"$REPORT" 2>&1 || true
tail -n 40 "$HOOKLOG" >>"$REPORT" 2>&1 || true

##############################################################################
# 6) Git push (redeploy Render)
##############################################################################
if [ -d .git ]; then
  BRANCH="$(git rev-parse --abbrev-ref HEAD || echo main)"
  git add -A
  git commit -m "doctor: fix indent, add GET/POST /api/notes, map+fill author_fp, limiter default, local logs" || true
  git push -u --force-with-lease origin "$BRANCH" || warn "Push falló (revisa remoto/credenciales)"
fi

say "Reporte listo: .tmp/paste12_doctor_report.txt"
