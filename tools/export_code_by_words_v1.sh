...
mapfile -t FILES < <(
  { git ls-files 2>/dev/null || true; git ls-files --others --exclude-standard 2>/dev/null || true; } \
  | sort -u
)

# Excluir rutas/archivos que no son texto o no tiene sentido exportar
...
# ==== 2) Crear partes sin superar MAX_WORDS ====
part_idx=1
...
for f in "${FILES[@]}"; do
  ...
  # Palabras del header de este archivo (aprox)
  hdr_words=20

  # Palabras del contenido del archivo
  fw="$(wc -w < "$f" | tr -d ' ' || echo 0)"
  add_words=$(( fw + hdr_words ))

  # Si supera el límite y la parte ya tiene algo → abrir nueva parte
  ...
  # Caso extremo: el archivo por sí solo excede el límite y la parte está vacía
  if (( curr_words == 0 && add_words > MAX_WORDS )); then
    # Truncar el contenido por palabras para no pasar MAX_WORDS
    ...
    echo "TRUNCATE:$f (words=$fw -> $allow)" >> "$TRUNCATED_LIST"
    {
      ...
      echo "SHA256: $(( (command -v sha256sum >/dev/null && sha256sum -- "$f" | awk '{print $1}') || (command -v shasum >/dev/null && shasum -a 256 -- "$f" | awk '{print $1}') || echo 'NA' ))"
      echo "------------------------------------------------------------"
      # Stream por líneas contando palabras acumuladas
      sed 's/\r$//' -- "$f" 2>/dev/null \
      | awk -v limit="$allow" '
          { c+=NF; print; if (c>=limit) { exit } }
        '
      echo
  ...
  # Caso normal: cabe entero en la parte actual
  append_header_and_file "$part" "$f"
  curr_words=$(( curr_words + add_words ))
  file_count=$((file_count+1))
done

# Copia rápida de la PRIMERA parte (para herramientas que solo “leen la primera”)
...
SUMMARY="$OUTDIR/code-wdump-$TS-summary.txt"
{
  echo "== SUMMARY =="
  echo "timestamp_utc: $TS"
  git rev-parse HEAD >/dev/null 2>&1 && echo "git_head: $(git rev-parse HEAD)"
  git remote get-url origin >/dev/null 2>&1 && echo "origin: $(git remote get-url origin)"
