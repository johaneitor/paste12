...
[[ -f "$TGT" ]] && cp -f "$TGT" "${TGT}.${TS}.bak" || true
echo "[backup] ${TGT}.${TS}.bak"

python - <<'PY'
# Reescribe backend/__init__.py con un factory estable + fallback limpio
...
def create_app() -> Flask:
    ...
    return app
'''
io.open("backend/__init__.py", "w", encoding="utf-8").write(textwrap.dedent(code).lstrip())
PY

# Sanity compile
python -m py_compile backend/__init__.py && echo "[py_compile] OK"

echo "Hecho. Revisa wsgi.py que exporte 'application = create_app()'."
