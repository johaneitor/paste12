...
echo "[+] Escribiendo $TARGET (rutas siempre registradas + DB lazy + shim en memoria)…"
cat > "$TARGET" <<'PY'
...
git add -A
git commit -m "wsgiapp bridge V2: rutas /api/notes siempre registradas + DB lazy + shim en memoria" || true
git push -u --force-with-lease origin "$BRANCH"

echo
echo "[i] Tras el redeploy en Render, ejecuta:"
echo "    curl -s https://paste12-rmsk.onrender.com/api/health"
echo "    curl -s https://paste12-rmsk.onrender.com/api/debug-urlmap | jq .   # Debe listar bridge_list_notes y bridge_create_note"
echo "    curl -i -s 'https://paste12-rmsk.onrender.com/api/notes?page=1' | sed -n '1,120p'"
echo "    curl -i -s -X POST -H 'Content-Type: application/json' -d '{\"text\":\"remote-ok\",\"hours\":24}' https://paste12-rmsk.onrender.com/api/notes | sed -n '1,160p'"
