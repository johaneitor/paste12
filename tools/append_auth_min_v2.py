#!/usr/bin/env python3
import pathlib, textwrap, py_compile, shutil, sys
...
if "class _AuthEndpoints" in raw:
    try:
        ...
        print("AVISO: _AuthEndpoints presente pero no compila:", e)

BLOCK = textwrap.dedent("""
# ================== APPEND-ONLY: AUTH MIN (safe) ==================
# Tablas: auth_user, auth_session
# Endpoints: POST /api/auth/register | POST /api/auth/login | POST /api/auth/logout | GET /api/auth/me
# Hash: PBKDF2-HMAC-SHA256 con salt (pbkdf2$sha256$iters$salt$hash)
...
class _AuthEndpoints:
    ...
    def _make_session_cookie(self, token):
        return f"p12_s={token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000"  # 30 días
...
s = raw
# aseguramos salto de línea antes de insertar
if not s.endswith("\n"):
    s += "\n"
s += "\n" + BLOCK

bak = W.with_suffix(".py.auth_patch_v2.bak")
if not bak.exists():
...
try:
    py_compile.compile(str(W), doraise=True)
    print("patched: auth v2 + compile OK; backup:", bak)
except Exception as e:
