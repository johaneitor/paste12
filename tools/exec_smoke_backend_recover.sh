...
RETRIES="${RETRIES:-30}"   # ~30 intentos
...
echo "== exec_smoke_backend_recover @ $BASE =="

# 0) Asegurar que el smoke exista
if [[ ! -x tools/smoke_backend_recover.sh ]]; then
  _yel "tools/smoke_backend_recover.sh no existe o no es ejecutable; lo creo rápido…"
  cat > tools/smoke_backend_recover.sh <<'SMOKE'
#!/usr/bin/env bash
set -euo pipefail
BASE="${1:-https://paste12-rmsk.onrender.com}"
_red(){ printf "\033[31m%s\033[0m\n" "$*"; }
...
SMOKE
  chmod +x tools/smoke_backend_recover.sh
fi

# 1) Esperar a que __api_import_error sea 404 (deploy sano)
echo "-- Esperando a __api_import_error=404 (hasta $RETRIES intentos cada $SLEEP_SECS s) --"
i=0
while :; do
  code="$(curl -sS -o /dev/null -w '%{http_code}' "$BASE/__api_import_error" || true)"
  if [[ "$code" == "404" ]]; then
  ...
  if [[ "$i" -ge "$RETRIES" ]]; then
    _red "Timeout esperando __api_import_error=404 (último status=$code)"
    exit 1
  fi
  printf "aún=%s  " "$code"
  sleep "$SLEEP_SECS"
...
# 2) Correr el smoke completo
tools/smoke_backend_recover.sh "$BASE"
