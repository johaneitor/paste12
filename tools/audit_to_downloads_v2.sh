...
# Uso:
#   tools/audit_to_downloads_v2.sh https://host [TTL_CAP_ESPERADO]
# Ej:
#   tools/audit_to_downloads_v2.sh "https://paste12-rmsk.onrender.com" 2160
...
# curl helpers que SIEMPRE devuelven body + code (sin -f)
# Devuelven: code en stdout (última línea) y dejan body/headers en paths opcionales
curl_code_body(){
...
pick_id="${FORM_ID:-$JSON_ID}"
tmp_get="$(mktemp)"
code="$(curl_code_body GET "$BASE/api/notes/$pick_id" "$tmp_get")"
if [ "$code" = "200" ]; then
...
PY
    ...
    echo "(aviso) faltan campos timestamp/expires_at en body:"
    cat "$tmp_get"
  fi
else
  bad "GET /api/notes/$pick_id → $code"
  cat "$tmp_get"
fi
hr
...
# same-FP
tmp_like1="$(mktemp)"; code="$(curl_json_code POST "$BASE/api/notes/$NEW/like" "$tmp_like1")"
L1="$(cat "$tmp_like1" | jget '.likes')"; D1="$(cat "$tmp_like1" | jget '.deduped')"
tmp_like2="$(mktemp)"; code2="$(curl_json_code POST "$BASE/api/notes/$NEW/like" "$tmp_like2")"
L2="$(cat "$tmp_like2" | jget '.likes')"; D2="$(cat "$tmp_like2" | jget '.deduped')"
echo "same-FP: ${L1:-?} ${D1:-?} -> ${L2:-?} ${D2:-?}"
if [ -n "${L1:-}" ] && [ "$L1" = "$L2" -o "${D2:-}" = "true" ]; then ok "dedupe OK"; else bad "dedupe anómalo"; fi

# FPs distintas
tmp_a="$(mktemp)"; ka="$(curl_json_code POST "$BASE/api/notes/$NEW/like" "$tmp_a")"
A="$(cat "$tmp_a" | jget '.likes')"
tmp_b="$(mktemp)"; kb="$(curl_json_code POST "$BASE/api/notes/$NEW/like" "$tmp_b")"
B="$(cat "$tmp_b" | jget '.likes')"
tmp_c="$(mktemp)"; kc="$(curl_json_code POST "$BASE/api/notes/$NEW/like" "$tmp_c")"
C="$(cat "$tmp_c" | jget '.likes')"
echo "A/B/C (HTTP): $ka $kb $kc"
echo "A/B/C (likes): ${A:-?} -> ${B:-?} -> ${C:-?}"

# Concurrencia ligera (5 peticiones, sin -f)
tmp_before="$(mktemp)"; code="$(curl_code_body GET "$BASE/api/notes/$NEW" "$tmp_before")"
...
wait || true
tmp_after="$(mktemp)"; code="$(curl_code_body GET "$BASE/api/notes/$NEW" "$tmp_after")"
...
if [ "$pg_code" = "200" ]; then
  ...
  if grep -qiE 'UndefinedColumn|author_fp' "$tmp_pg_body"; then
    echo "!! SCHEMA WARNING: columna faltante detectada (p. ej., author_fp)."
  fi
else
