...
CANDS = [pathlib.Path("backend/static/index.html"), pathlib.Path("frontend/index.html")]
MARK_A = "<!-- P12 CONSOLIDATED HOTFIX v6 (safe, idempotent) -->"
MARK_B = "<!-- /P12 CONSOLIDATED HOTFIX v6 -->"
...
// matar banners de "nueva versión disponible"
(function(){
...
(function(){ // oculta algún id común si existiera
...
function cardHTML(it){
  ...
  return `
    <article class="note" data-id="${it.id}">
      ...
      <div class="meta">#${it.id} · ❤ ${l} · <span class="views">👁 ${v}</span>
...
(function(){
  ...
  function attach(){
    document.querySelectorAll('article.note[data-id]').forEach(el=>{
      ...
      // fallback por click (primera interacción)
...
// ===== paginación keyset (Link/X-Next-Cursor)
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // arranque normal: primera página
...
def patch_one(p: pathlib.Path) -> bool:
    ...
    html = p.read_text(encoding="utf-8", errors="ignore").replace("\r\n","\n").replace("\r","\n")
    backup = p.with_suffix(p.suffix + ".p12_v6.bak")
    if MARK_A in html and MARK_B in html:
        ...
        new = re.sub(re.escape(MARK_A)+r".*?"+re.escape(MARK_B),
                     ...
                     html, flags=re.S)
        if new == html:
            print(f"OK: {p} ya estaba actualizado (v6).")
            return True
        if not backup.exists(): shutil.copyfile(p, backup)
        p.write_text(new, encoding="utf-8")
        print(f"patched: {p} (reemplazo v6) | backup={backup.name}")
        return True
    # insertar antes de </body>
    ins = MARK_A + JS + "\n" + MARK_B + "\n"
    if "</body>" in html.lower():
        # buscar la última ocurrencia case-insensitive
        m = re.search(r"</body\s*>", html, flags=re.I|re.S)
        pos = m.start() if m else len(html)
        new = html[:pos] + ins + html[pos:]
    else:
        new = html + "\n" + ins
    if not backup.exists(): shutil.copyfile(p, backup)
    p.write_text(new, encoding="utf-8")
    print(f"patched: {p} (inyección v6) | backup={backup.name}")
    return True

touched = False
for c in CANDS:
    try:
        if patch_one(c): touched = True
    except Exception as e:
        print(f"✗ error patch {c}: {e}")

if not touched:
    print("✗ No encontré index.html en backend/static/ o frontend/. ¿Ruta distinta?")
    sys.exit(2)
print("✓ listo")
