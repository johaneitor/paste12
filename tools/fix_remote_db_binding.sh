...
[ -f "$TARGET" ] || { echo "[!] No existe $TARGET (asegúrate de tener el paquete wsgiapp)."; exit 1; }

echo "[+] Backup de $TARGET"
cp -f "$TARGET" "$TARGET.bak.$(date +%s)"

echo "[+] Reescribiendo $TARGET con DB local ligada a la app (sin backend.db)…"
cat > "$TARGET" <<'PY'
...
echo "[+] Commit & push"
git add -A
git commit -m "fix(wsgiapp bridge): bind SQLAlchemy to the running app (no backend.db cross-binding)" || true
git push -u --force-with-lease origin "$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo main)"

echo
echo "[i] Luego, espera el redeploy de Render y prueba:"
