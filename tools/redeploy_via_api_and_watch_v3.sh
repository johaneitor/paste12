...
SERVICE_ID="${RENDER_SERVICE_ID:?export RENDER_SERVICE_ID=srv-...}"
...
api_post(){ local url="$1"; local body="${2:-{}}"
  while IFS= read -r ip; do
    if curl -sS --resolve "${HOST}:${PORT}:${ip}" -X POST \
      -H "Authorization: Bearer ${API_KEY}" -H "Content-Type: application/json" \
      -d "$body" "$url" -o /dev/null; then return 0; fi
  done < <(doh_ips); return 1; }

echo "== API: trigger deploy =="
api_post "https://${HOST}/v1/services/${SERVICE_ID}/deploys" "{}" || { echo "ERROR: no pude crear el deploy vía API"; exit 2; }
