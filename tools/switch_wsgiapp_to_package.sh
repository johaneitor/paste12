...
echo "[+] Preparando paquete wsgiapp/"
mkdir -p wsgiapp

echo "[+] Escribiendo wsgiapp/__init__.py (bridge → wsgi:app + debug + /api/notes fallback)"
cat > wsgiapp/__init__.py <<'PY'
from __future__ import annotations
import os, hashlib
from datetime import datetime, timedelta

# 1) Intenta reexportar la app real
try:
    from wsgi import app as app  # preferido
except Exception:
    # Fallback: app mínima
...
# 2) Blueprint puente con debug y /api/notes si faltan
...
# 3) Si /api/notes no existe, agregamos un fallback mínimo
if not (_has("/api/notes","GET") and _has("/api/notes","POST")):
    try:
        # Intentamos usar SQLAlchemy real si está configurado
        Note = None
        db = None
        try:
            ...
            # Creamos un SQLAlchemy mínimo local si no existía
        ...
        pass  # no rompemos si falla el fallback
...
# 5) Si existe wsgiapp.py (módulo), lo retiramos para que Python cargue el paquete
if [ -f wsgiapp.py ]; then
  echo "[+] Renombrando wsgiapp.py → wsgiapp.py.bak (para forzar import del paquete)"
  mv -f wsgiapp.py "wsgiapp.py.bak.$(date +%s)"
fi

echo "[+] Commit & push"
BRANCH="$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo main)"
git add -A
git commit -m "wsgiapp → paquete: bridge a wsgi:app + /api/debug-urlmap y /api/notes fallback" || true
git push -u --force-with-lease origin "$BRANCH"

cat <<'NEXT'

Hecho.
Ahora valida remoto con:
...
Si /api/bridge-ping devuelve JSON y /api/debug-urlmap lista reglas,
ya quedó activo el bridge. Si sigue 404, ajusta el Start Command a:
  gunicorn -w ${WEB_CONCURRENCY:-2} -k gthread --threads ${THREADS:-4} \
          -b 0.0.0.0:$PORT wsgiapp:app
(o alternativamente a wsgi:app). 
NEXT
