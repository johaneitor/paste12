...
echo "[+] Escribiendo wsgiapp.py (bridge + fuerza /api/notes si faltan + debug-urlmap + bridge-ping)"
cat > wsgiapp.py <<'PY'
...
git add -A
git commit -m "wsgiapp bridge v2: fuerza /api/notes si faltan + debug-urlmap + bridge-ping" || true
git push -u --force-with-lease origin "$BRANCH"

echo
echo "[i] Deja el Start Command en Render como:"
