...
echo "[+] Reescribiendo wsgiapp/__init__.py (bridge robusto)…"
mkdir -p wsgiapp
...
# 1) Obtener la app base, en orden: wsgi.app -> render_entry.app -> fallback Flask
app = None
try:
    ...
    try:
        from render_entry import app as _app2
        app = _app2
    except Exception:
        app = Flask(__name__)
        @app.get("/api/health")
...
# 2) Blueprint puente con debug + (si faltan) /api/notes
bp = Blueprint("bridge_probe", __name__)

@bp.get("/bridge-ping")
def bridge_ping():
    return jsonify(ok=True, where="wsgiapp.__init__", import_name=app.import_name), 200
...
# 3) Fallback /api/notes solo si NO existen
...
git add -A
git commit -m "wsgiapp bridge: robust import, always register /api/bridge-ping & /api/debug-urlmap; fallback /api/notes" || true
git push -u --force-with-lease origin "$BRANCH"

cat <<'NEXT'

Hecho. En Render, deja el Start Command como:
  gunicorn -w ${WEB_CONCURRENCY:-2} -k gthread --threads ${THREADS:-4} -b 0.0.0.0:$PORT wsgiapp:app

Luego verifica:
  curl -s https://paste12-rmsk.onrender.com/api/bridge-ping | jq .
...
Si /api/bridge-ping y /api/debug-urlmap devuelven JSON, el bridge está activo.
NEXT
