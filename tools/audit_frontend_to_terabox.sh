...
# 0) Permisos de almacenamiento (no falla si ya está hecho)
command -v termux-setup-storage >/dev/null 2>&1 && termux-setup-storage >/dev/null 2>&1 || true

# 1) Destino fijo en "Download/terabox" con varios paths posibles
pick_dest() {
  for d in \
    "/sdcard/Download/terabox" \
    "/storage/emulated/0/Download/terabox" \
    "$HOME/storage/downloads/terabox" \
    "$HOME/Download/terabox" \
    "$HOME/downloads/terabox"
  do
    mkdir -p "$d" 2>/dev/null || true
    if [ -d "$d" ] && [ -w "$d" ]; then echo "$d"; return; fi
  done
  # Último recurso
  mkdir -p "$HOME/downloads/terabox"
  echo "$HOME/downloads/terabox"
}
DEST="$(pick_dest)"
...
# 6) Resumen
{
  ...
  echo "- Si *uses_sw>0*: un SW viejo puede romper UI; probá con \`?nosw=1\`."
  echo
...
echo "OK"
echo "Resumen  => $OUT_SUM"
echo "HTML     => $RAW_HTML"
echo "JS list  => $JS_LIST"
