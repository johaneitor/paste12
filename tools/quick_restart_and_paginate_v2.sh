...
echo "➤ Kill & clean (posibles servers previos)" >&2
...
echo "➤ Esperando health 200 en $BASE…" >&2
for i in {1..30}; do
  code="$(curl -sS -o /dev/null -w '%{http_code}' "$BASE/api/health" || true)"
  [ "$code" = "200" ] && { echo "OK health=200" >&2; break; }
  sleep 1
done

echo "➤ Inyecto $SEED notas dummy para asegurar 2+ páginas" >&2
...
PY
  ...
  # Emitir SOLO el cursor (si existe) por stdout:
  tr -d '\r' < "$H" | awk -F': ' 'tolower($1)=="x-next-after"{print $2}' | tr -d '\r\n'
}

echo "➤ Página 1 (limit=$LIMIT)" >&2
