...
def iter_files():
    ...
    for root in ROOTS:
        ...
        for dirpath,_,files in os.walk(root):
            if any(x in dirpath for x in ('node_modules','venv','.git')): continue
            for f in files:
...
if __name__=='__main__':
    out = sys.argv[1] if len(sys.argv)>1 else os.path.join(os.path.expanduser('~'), 'Download', f'clones-{time.strftime("%Y%m%d-%H%M%SZ", time.gmtime())}.txt')
