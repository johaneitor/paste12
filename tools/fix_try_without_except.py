...
while i < len(lines):
    ...
    # Busca la siguiente línea "significativa" con indentación <= base
    j = i + 1
    while j < len(lines) and lines[j].strip() == "":
        j += 1

    # Avanza dentro del cuerpo del try hasta topar una línea al mismo nivel (o EOF)
    k = j
    while k < len(lines):
        l = lines[k]
        if l.strip() == "":
    ...
    # k es el primer índice con indent <= base (o == len(lines) si EOF)
    if k < len(lines):
        header = lines[k].lstrip()
        if re.match(r'^(except\b|finally\b)', header):
            # try ya tiene except/finally: no tocamos
            i = k + 1
            continue
        # insertar except antes de k
        lines.insert(k, base + "    pass")
        lines.insert(k, base + "except Exception:")
        changed = True
        # saltamos tras lo insertado
        i = k + 2
    else:
        # EOF: agregar except/pass al final
...
out = "\n".join(lines)
if changed:
    bak = W.with_suffix(".py.fix_try_no_except.bak")
    if not bak.exists():
        shutil.copyfile(W, bak)
    W.write_text(out, encoding="utf-8")
    print(f"patched: inserted missing except/pass for bare try | backup={bak.name}")
else:
