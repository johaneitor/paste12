...
# Normalización dura de EOL y tabs
s = raw.replace("\r\n","\n").replace("\r","\n").replace("\t","    ")
changed = (s != raw)

# 1) Borrar TODAS las definiciones (enteras o truncadas) de _json_passthrough_like
lines = s.split("\n")
i = 0
removed = 0
while i < len(lines):
    ...
    start = i
    i += 1
    # Consumir "cuerpo" hasta encontrar un inicio de bloque al mismo nivel o menor,
    # o fin de archivo
    while i < len(lines):
        line = lines[i]
        if line.strip() == "":
        ...
        # Si dedenta (<=) y luce como inicio de algo top-level / bloque nuevo
...
# 2) Si aún quedara algún header perdido (casos raros), cámbialo por un stub válido
...
# 3) Asegurar que exista UNA versión sana a nivel módulo (si ya hay una, no duplicar)
if "def _json_passthrough_like(" not in s or "pass" in s.split("def _json_passthrough_like(",1)[1].split("\n",1)[0]:
    helper = '''
def _json_passthrough_like(note_id: int):
    """
    Helper estable: incrementa likes y devuelve payload normalizado.
    Side-effect único: UPDATE cuando aplica. Seguro ante errores.
    """
    try:
...
if not changed:
    print("Nada que cambiar; ya parecía sano.")

# Backup y escritura
bak = W.with_suffix(".py.pre_likefix3.bak")
if not bak.exists():
...
try:
    ...
    print("Backup:", bak)
    # para inspección rápida:
    with open(W, "r", encoding="utf-8") as fh:
        ctx = fh.read().split("\n")
    for ln in range(300, 340):
        if ln-1 < len(ctx):
