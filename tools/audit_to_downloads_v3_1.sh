...
# Uso:
#   tools/audit_to_downloads_v3_1.sh https://host [TTL_CAP_ESPERADO]
# Ej:
#   tools/audit_to_downloads_v3_1.sh "https://paste12-rmsk.onrender.com" 2160
...
pick_working_id(){
  # Prueba GET /api/notes/:id y devuelve el primero que responda 200
  local a="$1" b="$2" tmp="$(mktemp)" c
  if [ -n "$a" ]; then
    c="$(curl_code_body GET "$BASE/api/notes/$a" "$tmp")"; [ "$c" = "200" ] && { echo "$a"; return; }
  fi
  if [ -n "$b" ]; then
    c="$(curl_code_body GET "$BASE/api/notes/$b" "$tmp")"; [ "$c" = "200" ] && { echo "$b"; return; }
  fi
  echo ""
}
...
SELECT_ID="$(pick_working_id "$FORM_ID" "$JSON_ID")"
if [ -z "$SELECT_ID" ]; then
  bad "ningún id responde 200 en GET /api/notes/:id; corto TTL/likes/paginación"
  echo "Archivo escrito en: $OUT"
  exit 1
fi
echo "id elegido para TTL/likes/concurrencia: $SELECT_ID"
hr

echo "== TTL (GET /api/notes/:id) =="
tmp_get="$(mktemp)"
gcode="$(curl_code_body GET "$BASE/api/notes/$SELECT_ID" "$tmp_get")"
if [ "$gcode" = "200" ]; then
...
PY
  ...
  bad "GET /api/notes/$SELECT_ID → $gcode"
  cat "$tmp_get"
fi
hr
...
# same-FP
tmp_like1="$(mktemp)"; l1c="$(curl_json_code POST "$BASE/api/notes/$SELECT_ID/like" "$tmp_like1")"
L1="$(cat "$tmp_like1" | jget '.likes')"; D1="$(cat "$tmp_like1" | jget '.deduped')"
tmp_like2="$(mktemp)"; l2c="$(curl_json_code POST "$BASE/api/notes/$SELECT_ID/like" "$tmp_like2")"
...
# FPs distintas
tmp_a="$(mktemp)"; ka="$(curl -sS -X POST -H 'X-FP: A' -o "$tmp_a" -w '%{http_code}' "$BASE/api/notes/$SELECT_ID/like")"
A="$(cat "$tmp_a" | jget '.likes')"
tmp_b="$(mktemp)"; kb="$(curl -sS -X POST -H 'X-FP: B' -o "$tmp_b" -w '%{http_code}' "$BASE/api/notes/$SELECT_ID/like")"
B="$(cat "$tmp_b" | jget '.likes')"
tmp_c="$(mktemp)"; kc="$(curl -sS -X POST -H 'X-FP: C' -o "$tmp_c" -w '%{http_code}' "$BASE/api/notes/$SELECT_ID/like")"
C="$(cat "$tmp_c" | jget '.likes')"
echo "A/B/C (HTTP): $ka $kb $kc"
echo "A/B/C (likes): ${A:-?} -> ${B:-?} -> ${C:-?}"

# Concurrencia ligera (5 peticiones FP=Z)
tmp_before="$(mktemp)"; gb="$(curl_code_body GET "$BASE/api/notes/$SELECT_ID" "$tmp_before")"
BEFORE="$(cat "$tmp_before" | jget '.item.likes')"
tmp_codes="$(mktemp)"
for i in $(seq 1 5); do
  (curl -sS -o /dev/null -w '%{http_code}\n' -X POST "$BASE/api/notes/$SELECT_ID/like" -H 'X-FP: Z' >> "$tmp_codes") &
  jitter
done
wait || true
tmp_after="$(mktemp)"; ga="$(curl_code_body GET "$BASE/api/notes/$SELECT_ID" "$tmp_after")"
...
if [ "$pg_code" = "200" ]; then
  ...
  # shape mínimo
  CNT="$(jq -r '.items|length' "$tmp_pg_body" 2>/dev/null || echo 0)"
  echo "items: $CNT"
  jq -r '.items[]|[.id,.timestamp,.expires_at]|@tsv' "$tmp_pg_body" 2>/dev/null \
    | awk -F'\t' '{print "id:",$1}' || cat "$tmp_pg_body"
else
