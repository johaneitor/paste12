...
for i in 1 2 3; do
  curl -sS -H 'Content-Type: application/json' -d '{"text":"check json v2","hours":24}' "$BASE/api/notes" >/dev/null || true
done
...
sed -n '1,/^$/d' "$TMP" | head -c 2000; echo
