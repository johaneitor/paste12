#!/usr/bin/env bash
set -euo pipefail
MSG="${1:-ops: corefix - stage deletions + add tools + pooling_guard + sanity}"
...
[[ -n "$up" ]] && echo "Remote: $up" || echo "Remote: (upstream recien configurado)"
