...
echo "[+] Escribiendo $TARGET con fallback /api/notes y SQLAlchemy(app) local…"
cat > "$TARGET" <<'PY'
...
git add -A
git commit -m "wsgiapp bridge: add forced /api/notes with local SQLAlchemy(app) + debug-urlmap" || true
git push -u --force-with-lease origin "$BRANCH"

echo
echo "[i] Tras el redeploy (Render), probá:"
