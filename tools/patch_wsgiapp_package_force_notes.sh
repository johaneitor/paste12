...
mkdir -p wsgiapp
echo "[+] Backup de wsgiapp/__init__.py (si existe)"
[ -f wsgiapp/__init__.py ] && cp -f wsgiapp/__init__.py "wsgiapp/__init__.py.bak.$(date +%s)" || true

echo "[+] Escribiendo wsgiapp/__init__.py (fuerza /api/notes + /api/debug-urlmap sobre la app real)"
...
git add -A
git commit -m "wsgiapp package: fuerza /api/notes y /api/debug-urlmap en __init__.py (idempotente)" || true
...
echo "[i] Tras el redeploy, valida:"
echo "    curl -s https://paste12-rmsk.onrender.com/api/bridge-ping | jq ."
