...
# Ignorar scripts locales si no los querés subir
{ grep -q '^tools/.*\.sh$' .gitignore 2>/dev/null || echo 'tools/*.sh'; } >> .gitignore

git add README.md .gitignore
git commit -m "docs: README con endpoints/ejemplos y ajustes de deploy; ignora scripts locales"
