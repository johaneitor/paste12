...
echo "[+] Backup previo de wsgi.py (si existe)"
[ -f wsgi.py ] && cp -f wsgi.py "wsgi.py.bak.$(date +%s)" || true                                               
echo "[+] Escribiendo wsgi.py (app con /api/notes y /api/debug-urlmap forzados)"                                cat > wsgi.py <<'PY'
from __future__ import annotations
import os, hashlib
from datetime import datetime, timedelta
from flask import Flask, Blueprint, jsonify, request

USING_FACTORY = False                                   app = None
db = None                                               Note = None
...
try:
    ...
    db = _db
    Note = _Note
    USING_FACTORY = True                                except Exception:
    pass
...
# 4) Blueprint forzado con endpoints únicos
...
# 5) Registrar siempre el blueprint forzado bajo /api
try:
...
git add -A
git commit -m "wsgi: hard entrypoint with forced /api/notes and /api/debug-urlmap + create_all()" || true
git push -u --force-with-lease origin "$BRANCH"

echo
echo "==============================================="
echo "[!] Ahora en Render > Service > Start Command:"
echo "    gunicorn -w \${WEB_CONCURRENCY:-2} -k gthread --threads \${THREADS:-4} -b 0.0.0.0:\$PORT wsgi:app"
echo "    (ojo: 'wsgi:app', NO 'wsgiapp:app')"
echo "==============================================="
