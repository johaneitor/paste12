...
echo "[+] Escribiendo render_entry.py (entrypoint único con guardas y fallback /api/notes)"
...
git add -A
git commit -m "deploy: render_entry.py (entrypoint único) + /api/debug-urlmap y fallback /api/notes" || true
git push -u --force-with-lease origin "$BRANCH"

echo
echo "==============================================================="
echo "[!] En Render > Service > Start Command, pon EXACTAMENTE:"
echo "    gunicorn -w \${WEB_CONCURRENCY:-2} -k gthread --threads \${THREADS:-4} -b 0.0.0.0:\$PORT render_entry:app"
echo "==============================================================="
echo
echo "Luego de redeploy, valida con:"
