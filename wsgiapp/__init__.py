...
# --- version stamp for health (autogenerated) ---
try:
    from flask import Blueprint, jsonify
    _vs = Blueprint("wsgiapp_version", __name__)
    import os, time
    _STAMP = os.getenv("DEPLOY_STAMP", str(int(time.time())))
    @_vs.get("/health-stamp")
    def _health_stamp():
        return jsonify(ok=True, note="wsgiapp", stamp=_STAMP), 200
    try:
        app.register_blueprint(_vs, url_prefix="/api")
    except Exception:
        pass
except Exception:
    pass
